...
import static com.jindam.base.code.AppointmentStartTypeCode.changeByCustomer;
...
import com.jindam.app.appointment.model.AppointmentDeleteRequestDto;
...
import com.jindam.app.appointment.model.AppointmentEmailRequestDto;
import com.jindam.app.appointment.model.AppointmentInsertRequestDto;
import com.jindam.app.appointment.model.AppointmentSignInsertRequestDto;
import com.jindam.app.appointment.model.AppointmentTreatmentDetailResponseDto;
import com.jindam.app.appointment.model.AppointmentTreatmentInsertRequestDto;
import com.jindam.app.appointment.model.AppointmentUpdateRequestDto;
...
public class AppointmentService extends PagingService {
    ...
    public PagingResponseDto<AppointmentDetailResponseDto> selectAppointmentByDesignerIdPaging(AppointmentDetailRequestDto request) {
        ...
        PagingResponseDto<AppointmentDetailResponseDto> pagingResult = findData(appointmentMapper, "selectAppointmentByDesignerIdPaging", request);
    ...
    public AppointmentDetailResponseDto insertAppointment(AppointmentInsertRequestDto request) {
        ...
        /*
         * to-do 예약성공시 예약 스타일 테이블 인서트 자동확정 여부에 따라서 분기처리 자동확정시 예약상태 확정으로업데이트 알림등록 (요청응답) 알림센터 인서트(고객디자이너 둘다) 알림 테이블 인서트 채팅 인서트 채팅방 없으면 생성하고 아닐시 예약변경으로 들어온 케이스 알림등록(요청응답), 알림센터 등록(고객디자이너 둘다) ,기존 알림 푸쉬삭제 ,예약 관련 알림 신규 푸쉬인서트, 채팅발송 신규 예약 케이스 알림등록(요청응답), 알림센터 등록(고객디자이너 둘다), 신규푸쉬인서트(전날 17시, 당일 2시간전) , 채팅발송
         */
        ...
        if (autoYn.equals("Y")) {
            ...
            if ((request.getAppointmentStartTypeCode() == changeByCustomer) || (request.getAppointmentStartTypeCode() == AppointmentStartTypeCode.changeByDesigner)) {
                // 기존 에약건에 대한 푸쉬 알림 삭제
    ...
    public void confirmAppointment(AppointmentInsertRequestDto request) {
        ...
        AppointmentUpdateRequestDto updateDto = AppointmentUpdateRequestDto.builder()
            .appointmentId(request.getAppointmentId())
            .appointmentStatusCode(AppointmentStatusCode.completed)
            .workAt(LocalDateTime.now())
            .workId(request.getDesignerUid())
            .build();

        updateDto.setAppointmentStatusCode(AppointmentStatusCode.completed);// 예약완료
        ...
        NotificationInsertCenterRequestDto notificationInsertCenterRequestDtoCustomer = NotificationInsertCenterRequestDto.builder()
            .notificationTopic("")
            .eventClick("")
            .notificationTypeCode(NotificationTypeCode.userAppointment) // 고객예약
        ...
        NotificationInsertCenterRequestDto notificationInsertCenterRequestDtoDesigner = NotificationInsertCenterRequestDto.builder()
            .notificationTopic("")
            .eventClick("")
            .notificationTypeCode(NotificationTypeCode.designerAppointment)// 디자이너예약
        ...
        if (!appointmentDate.equals(treatmentDate)) {
            ...
            if (sendTimeYesterday.isAfter(now)) {
                NotificationInsertPushRequestDto notificationInsertPushRequestDtoCustomerYesterday = NotificationInsertPushRequestDto.builder()
                    ...
                    .pushTitle("전날 푸쉬")
                    .pushTypeCode(PushTypeCode.appointment)
        ...
        ChatInsertRequestDto chatInsertRequestDtoCustomer = ChatInsertRequestDto.builder()
            ...
            .chatMessageContentTypeCode(ChatMessageContentTypeCode.basic) // 일반
            .chatMessageTypeCode(ChatMessageTypeCode.txt) // 텍스트
    ...
    public void updateAppointment(AppointmentUpdateRequestDto request) {
        ...
        AppointmentUpdateRequestDto updateDto = AppointmentUpdateRequestDto.builder()
            .appointmentId(request.getAppointmentId())
            .appointmentStatusCode(AppointmentStatusCode.requested)// 예약요청
            .workAt(LocalDateTime.now())
            .workId(request.getDesignerUid())
            .build();
        ...
        NotificationInsertCenterRequestDto notificationInsertCenterRequestDtoCustomer = NotificationInsertCenterRequestDto.builder()
            .notificationTopic("")
            .eventClick("")
            .notificationTypeCode(NotificationTypeCode.userModifyAppointment) // 고객수정
            .notificationTitle("알림센터_고객")
        ...
        NotificationInsertCenterRequestDto notificationInsertCenterRequestDtoDesigner = NotificationInsertCenterRequestDto.builder()
            .notificationTopic("")
            .eventClick("")
            .notificationTypeCode(NotificationTypeCode.designerModifyAppointment)// 디자이너수정
            .notificationTitle("알림센터_디자이너")
            .notificationContent("예약 수정됨")
            .receiverUid(request.getDesignerUid())
            .designerName(request.getDesignerName())
    ...
    public void deleteAppointment(AppointmentDeleteRequestDto request) {
        ...
        int chatResult = 0;
        if (request.getAppointmentStartTypeCode() == changeByCustomer) {
